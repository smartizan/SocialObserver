...
class VKDBC:
	...
	def getMostActive(self):
		...
		conn = sqlite3.connect(self.db_puth)
		c = conn.cursor()
		sql = "SELECT COUNT(activity.pid) AS c, users.uid,  users.first_name, users.last_name  FROM activity JOIN users ON activity.uid=users.uid GROUP BY activity.uid ORDER BY c DESC;"
		c.execute(sql)
		rows = c.fetchall()
		c.close()
		conn.close()
	...
	def getTwoDays(self):
		''' Получить песоледний и предпоследний дни. Определять через дату плохо в случае, если сделаны пропуски в исследованиях '''
		conn = sqlite3.connect(self.db_puth)
		c = conn.cursor()
		sql = 'SELECT users_groups.date FROM users_groups GROUP BY users_groups.date ORDER BY users_groups.date DESC;'
		c.execute(sql)
		rows = c.fetchall()
		# ~ print(rows[0])
		# ~ print(rows[1])
